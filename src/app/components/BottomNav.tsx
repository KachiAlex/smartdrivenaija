...
export function BottomNav({ activeTab, onTabChange }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-4 left-4 right-4 z-50" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
      <div
        className="relative flex items-center justify-between px-2 py-2 rounded-[2rem] shadow-2xl shadow-[#14532D]/10 border border-white/20"
        ...
        {navItems.map((item) => {
          ...
          return (
            <motion.button
              ...
              style={{ color: isActive ? "#ffffff" : "#4B7C5F" }}
              whileTap={{ scale: 0.9 }}
            >
              <motion.div
