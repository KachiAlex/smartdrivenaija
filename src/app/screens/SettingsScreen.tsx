...
function SectionRow({
  ...
  return (
    <button
      onClick={onToggle}
      className="w-full flex items-center gap-3 p-4 text-left hover:bg-[#F0FDF4] transition-colors"
    >
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${open ? "bg-[#15803D] text-white" : "bg-[#F0FDF4] text-[#4B7C5F]"}`}>
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-sm text-[#14532D]">{label}</p>
        {description && <p className="text-xs text-[#4B7C5F]">{description}</p>}
      </div>
      <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }}>
        <ChevronDown className="w-4 h-4 text-[#4B7C5F]" />
      </motion.div>
    </button>
  );
}
...
function ChangePasswordPanel() {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-4 pb-4 pt-2 space-y-3 bg-[#F0FDF4]">
      <div className="relative">
        <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4B7C5F]" />
        <Input
          type={show ? "text" : "password"}
          placeholder="Current password"
          value={current}
          onChange={e => setCurrent(e.target.value)}
          className="pl-10 h-11 border-2 border-[#BBF7D0] bg-white rounded-xl text-sm"
          required
        />
      </div>
      <div className="space-y-1.5">
        <div className="relative">
          ...
          <Input
            ...
            className="pl-10 pr-10 h-11 border-2 border-[#BBF7D0] bg-white rounded-xl text-sm"
            required
          />
          <button type="button" onClick={() => setShow(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#4B7C5F]">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className={`pl-10 h-11 border-2 bg-white rounded-xl text-sm ${confirm && confirm !== next ? "border-red-400" : "border-[#BBF7D0]"}`}
          required
        />
      </div>
...
function BiometricsPanel() {
  ...
  return (
    <div className="px-4 pb-4 pt-2 space-y-3 bg-[#F0FDF4]">
      <p className="text-xs text-[#4B7C5F] leading-relaxed">
      ...
      {loading ? (
        ...
        <div className="text-center py-6 space-y-2">
          <Fingerprint className="w-10 h-10 text-[#4B7C5F] mx-auto" />
          <p className="text-sm text-[#4B7C5F]">No biometric credentials registered</p>
          <p className="text-xs text-[#4B7C5F]">Use the mobile app to register your device</p>
        </div>
      ) : (
        <div className="space-y-2">
          {credentials.map(c => (
            <div key={c.credentialId} className={`bg-white rounded-xl border-2 p-3 flex items-center gap-3 ${c.isActive ? "border-[#BBF7D0]" : "border-gray-100 opacity-60"}`}>
              <div className="w-10 h-10 rounded-xl bg-[#F0FDF4] flex items-center justify-center text-xl shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#4B7C5F]">
                  {c.biometricType || c.deviceType || "Biometric"} · {c.isActive ? "Active" : "Inactive"}
                </p>
                {c.lastUsedAt && (
                  <p className="text-xs text-[#4B7C5F]">Last used {new Date(c.lastUsedAt).toLocaleDateString()}</p>
                )}
              </div>
...
function AccountPanel({ onNavigate }: { onNavigate?: (s: string) => void }) {
  ...
  return (
    <form onSubmit={handleSave} className="px-4 pb-4 pt-2 space-y-3 bg-[#F0FDF4]">
      <div>
        <label className="block text-xs font-semibold text-[#4B7C5F] uppercase tracking-wide mb-1">Full Name</label>
        <div className="relative">
      ...
      <div>
        <label className="block text-xs font-semibold text-[#4B7C5F] uppercase tracking-wide mb-1">Phone</label>
        <div className="relative">
          <Smartphone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4B7C5F]" />
          <Input
            value={user?.phone || ""}
            disabled
            className="pl-10 h-11 border-2 border-[#BBF7D0] bg-gray-50 rounded-xl text-sm text-[#4B7C5F]"
          />
        </div>
      </div>
      <div>
        <label className="block text-xs font-semibold text-[#4B7C5F] uppercase tracking-wide mb-1">Preferred Language</label>
        <div className="relative">
          <Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4B7C5F]" />
          <select
            value={lang}
            onChange={e => setLang(e.target.value)}
            className="w-full pl-10 pr-4 h-11 border-2 border-[#BBF7D0] bg-white rounded-xl text-sm appearance-none focus:border-[#15803D] outline-none"
          >
...
function AppearancePanel() {
  ...
  return (
    <div className="px-4 pb-4 pt-2 space-y-1 bg-[#F0FDF4]">
      <div className="flex items-center justify-between py-3">
        <div className="flex items-center gap-3">
          <Moon className="w-4 h-4 text-[#4B7C5F]" />
          <Label className="text-sm font-medium text-[#14532D] cursor-pointer">Dark Mode</Label>
        </div>
      ...
      <div className="flex items-center justify-between py-3">
        <div className="flex items-center gap-3">
          <Bell className="w-4 h-4 text-[#4B7C5F]" />
          <Label className="text-sm font-medium text-[#14532D] cursor-pointer">Push Notifications</Label>
        </div>
      ...
      <div className="flex items-center justify-between py-3">
        <div className="flex items-center gap-3">
          <Bell className="w-4 h-4 text-[#4B7C5F]" />
          <Label className="text-sm font-medium text-[#14532D] cursor-pointer">Study Reminders</Label>
        </div>
...
export function SettingsScreen({ onNavigate }: { onNavigate?: (screen: string) => void }) {
  ...
  const sections = [
    ...
    },
    {
      ...
      content: (
        <div className="px-4 pb-4 pt-2 space-y-3 bg-[#F0FDF4]">
          <div className="bg-white rounded-xl border border-[#BBF7D0] divide-y">
            <div className="p-3 flex items-start gap-3">
              <Shield className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
              <div>
                <p className="text-sm font-semibold text-[#14532D]">End-to-End Encrypted</p>
                <p className="text-xs text-[#4B7C5F] mt-0.5">All your data is encrypted in transit and at rest.</p>
              </div>
            </div>
            <div className="p-3 flex items-start gap-3">
              <Lock className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
              <div>
                <p className="text-sm font-semibold text-[#14532D]">JWT Session Tokens</p>
                <p className="text-xs text-[#4B7C5F] mt-0.5">Sessions expire automatically after 24 hours of inactivity.</p>
              </div>
            </div>
          </div>
          <Button
  ...
  return (
    <div className="size-full overflow-auto bg-[#F0FDF4] pb-28">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #15803D 0%, #0A1F12 100%)" }}
      >
        <motion.div
          animate={{ scale: [1, 1.3, 1], x: [0, 30, 0] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          className="absolute -top-20 -right-20 w-60 h-60 rounded-full bg-[#22C55E]/20 blur-[70px]"
        />
        <motion.div
        ...
        >
          ...
          <div>
            ...
            <span className={`mt-1 inline-block text-xs px-2 py-0.5 rounded-full font-semibold ${user?.isPremium ? "bg-[#22C55E] text-white" : "bg-white/20 text-white/80"}`}>
      ...
      <div className="p-5 -mt-4 relative z-10 space-y-3">
        {/* Settings sections */}
        <Card className="overflow-hidden glass-card border-[#15803D]/10 divide-y divide-[#BBF7D0]">
          {sections.map(s => (
            <div key={s.id}>
              <SectionRow
                icon={s.icon}
        ...
        {/* App version */}
        <p className="text-center text-xs text-[#4B7C5F] pb-2">SmartDrive Naija v1.0.0</p>
      </div>
    </div>
  );
}
