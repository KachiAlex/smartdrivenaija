...
export function LoginScreen({ onLogin, onForgotPassword, onRegister }: LoginScreenProps) {
  ...
  const inputCls = "pl-12 h-12 border-2 border-[#BBF7D0] bg-[#F0FDF4] focus:border-[#15803D]/50 focus:ring-[#15803D]/10 transition-all rounded-xl text-[#14532D] placeholder:text-[#4B7C5F]";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-[#0A1F12] overflow-hidden">
      ...
      <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.06, 0.12, 0.06] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} className="absolute top-[5%] right-[10%] w-56 h-56 rounded-full bg-[#22C55E]/15 blur-[60px]" />
      <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.05, 0.1, 0.05] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-[20%] left-[30%] w-64 h-64 rounded-full bg-[#15803D]/10 blur-[70px]" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 px-6">
        <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="mb-6">
          <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-[#15803D] to-[#22C55E] flex items-center justify-center shadow-lg shadow-[#15803D]/20">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.5 }} className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[#4ADE80] text-sm font-extrabold tracking-[0.15em]" style={{ fontFamily: "Poppins" }}>NAIJA</span>
          <div className="h-[2px] w-6 bg-white/30 rounded-full" />
        </motion.div>
      ...
      >
        <div className="mb-5">
          <h2 className="text-[#14532D] text-xl font-bold mb-1" style={{ fontFamily: "Poppins" }}>Welcome Back</h2>
          <p className="text-[#4B7C5F] text-sm">Sign in to continue your journey</p>
        </div>

        {/* Mode toggle */}
        <div className="flex bg-[#F0FDF4] rounded-xl p-1 mb-5">
          <button
            type="button"
            onClick={() => setMode('password')}
            className={`flex-1 flex items-center justify-center gap-2 h-9 rounded-lg text-sm font-semibold transition-all ${mode === 'password' ? 'bg-white text-[#14532D] shadow-sm' : 'text-[#4B7C5F]'}`}>
            <Lock className="w-3.5 h-3.5" /> Password
          </button>
          <button
            type="button"
            onClick={() => setMode('otp')}
            className={`flex-1 flex items-center justify-center gap-2 h-9 rounded-lg text-sm font-semibold transition-all ${mode === 'otp' ? 'bg-white text-[#14532D] shadow-sm' : 'text-[#4B7C5F]'}`}>
          ...
          {mode === 'password' ? (
            ...
            >
              ...
              <div className="space-y-2">
                <Label htmlFor="identifier" className="text-xs font-semibold text-[#4B7C5F] uppercase tracking-wider">Phone or Email</Label>
              ...
              {/* Password */}
              <div className="space-y-2">
                <Label htmlFor="password" className="text-xs font-semibold text-[#4B7C5F] uppercase tracking-wider">Password</Label>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#4B7C5F] hover:text-[#15803D] transition-colors">
              ...
              <div className="flex justify-end">
                <button type="button" onClick={onForgotPassword} className="text-xs font-semibold text-[#15803D] hover:text-[#22C55E] transition-colors">
                  Forgot password?
                </button>
              </div>
            ...
            >
              {/* OTP channel toggle */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold text-[#4B7C5F] uppercase tracking-wider">Receive code via</Label>
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setOtpChannel('sms')} className={`flex items-center justify-center gap-2 h-11 rounded-xl border-2 transition-all font-medium text-sm ${otpChannel === 'sms' ? 'border-[#15803D] bg-[#15803D]/5 text-[#15803D]' : 'border-[#BBF7D0] bg-[#F0FDF4] text-[#4B7C5F]'}`}>
                    <MessageSquare className="w-4 h-4" /> SMS
                  </button>
                  <button type="button" onClick={() => setOtpChannel('email')} className={`flex items-center justify-center gap-2 h-11 rounded-xl border-2 transition-all font-medium text-sm ${otpChannel === 'email' ? 'border-[#15803D] bg-[#15803D]/5 text-[#15803D]' : 'border-[#BBF7D0] bg-[#F0FDF4] text-[#4B7C5F]'}`}>
              ...
              {otpChannel === 'sms' && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                  <Label htmlFor="otp-phone" className="text-xs font-semibold text-[#4B7C5F] uppercase tracking-wider">Phone Number</Label>
                  <div className="relative">
              ...
              {otpChannel === 'email' && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                  <Label htmlFor="otp-email" className="text-xs font-semibold text-[#4B7C5F] uppercase tracking-wider">Email Address</Label>
                  <div className="relative">
                    <AtSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#4B7C5F]" />
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-[#F0FDF4] border border-[#BBF7D0]">
                <div className="p-1.5 rounded-lg bg-[#15803D]/10 mt-0.5">
        ...
        {/* Register link */}
        <p className="text-center text-sm text-[#4B7C5F] mt-5">
          New to SmartDrive?{' '}
          <button type="button" onClick={onRegister} className="font-semibold text-[#15803D] hover:text-[#22C55E] transition-colors">
            Create account
          </button>
        </p>
      </motion.div>
